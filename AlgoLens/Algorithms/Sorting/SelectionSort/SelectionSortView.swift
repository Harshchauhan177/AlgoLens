            ...
            ScrollView {
                VStack(spacing: Theme.Spacing.large) {
                    ...
                    VStack(spacing: Theme.Spacing.small) {
                        ...
                        Text("Find minimum element and place it at the beginning")
                    ...
                    if !viewModel.isAnimating {
                        VStack(spacing: Theme.Spacing.medium) {
                            ...
                            VStack(alignment: .leading, spacing: Theme.Spacing.small) {
                                ...
                                TextField("e.g., 64,25,12,22,11", text: $viewModel.arrayInput)
                    ...
                    VStack(spacing: Theme.Spacing.medium) {
                        HStack {
                            ...
                            if viewModel.isAnimating || viewModel.isCompleted {
                                Text("Pass: \(viewModel.currentPass)/\(viewModel.totalPasses)")
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            ...
                            .padding(.horizontal, Theme.Spacing.large)
                            .padding(.vertical, Theme.Spacing.small)
                            .frame(height: 350) // Fixed height for consistent baseline
                        }
                    }
                    
                    // Step Information Panel
                    if viewModel.isAnimating || viewModel.isCompleted {
                        SelectionSortStepInformationPanel(
                            currentPass: viewModel.currentPass,
                            totalPasses: viewModel.totalPasses,
...
// MARK: - Selection Sort Step Information Panel
struct SelectionSortStepInformationPanel: View {
    let currentPass: Int
    let totalPasses: Int
    let currentStep: Int
    ...
    var body: some View {
        VStack(spacing: Theme.Spacing.medium) {
            ...
            VStack(spacing: Theme.Spacing.small) {
                // Current State
                if !isCompleted {
                    InfoRow(label: "Current Pass", value: "\(currentPass) of \(totalPasses)", icon: "arrow.triangle.2.circlepath", color: .purple)
                    InfoRow(label: "Comparisons", value: "\(currentStep)", icon: "number.circle.fill", color: .blue)
                    ...
                    if !comparisonResult.isEmpty {
                        ...
                        HStack {
                            Image(systemName: comparisonResult.contains("⚡") ? "bolt.circle.fill" : comparisonResult.contains("🔍") ? "magnifyingglass.circle.fill" : "checkmark.circle.fill")
                                .foregroundColor(comparisonResult.contains("⚡") ? .orange : comparisonResult.contains("🔍") ? .red : .green)
...
// MARK: - Selection Sort Array Element View
struct SelectionSortArrayElementView: View {
    let value: Int
    let index: Int
    let state: SelectionSortViewModel.ElementState
    ...
    var body: some View {
        VStack(spacing: Theme.Spacing.small) {
            ...
            RoundedRectangle(cornerRadius: 8)
                ...
                .frame(width: 50, height: CGFloat(value) * 3) // Height proportional to value
                .overlay(
                    RoundedRectangle(cornerRadius: 8)
                        .stroke(borderColor, lineWidth: borderWidth)
                )
                .shadow(color: shadowColor, radius: shadowRadius, x: 0, y: 4)
                .scaleEffect(state == .minimum || state == .current ? 1.05 : 1.0)
