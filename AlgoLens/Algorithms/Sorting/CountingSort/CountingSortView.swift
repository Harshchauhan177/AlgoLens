    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: Theme.Spacing.large) {
                    ...
                    VStack(spacing: Theme.Spacing.small) {
                        ...
                        Text("Count occurrences, then place in sorted order")
                    ...
                    if !viewModel.isAnimating {
                        VStack(spacing: Theme.Spacing.medium) {
                            ...
                            VStack(alignment: .leading, spacing: Theme.Spacing.small) {
                                Text("Array (comma-separated, non-negative integers ≤ 20)")
                    ...
                    // Count Array Visualization (when available)
                    if !viewModel.countArray.isEmpty {
                        VStack(spacing: Theme.Spacing.medium) {
                            HStack {
                                Text("Count Array")
                                    .font(.system(size: 16, weight: .semibold, design: .rounded))
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(alignment: .bottom, spacing: 8) {
                                    ForEach(Array(viewModel.countArray.enumerated()), id: \.offset) { index, count in
                                        CountArrayElementView(
                                            index: index,
                                            count: count,
                                            isHighlighted: viewModel.countElementState(at: index)
                                        )
                                    }
                                }
                                .padding(.horizontal, Theme.Spacing.large)
                    ...
                    if viewModel.isAnimating || viewModel.isCompleted {
                        CountingSortStepInformationPanel(
        ...
        .navigationDestination(isPresented: $showQuiz) {
            QuizView(algorithm: Algorithm(
                name: "Counting Sort",
                description: "Integer sorting using count array",
                icon: "arrow.up.arrow.down.circle.fill",
                complexity: Algorithm.Complexity(time: "O(n + k)", space: "O(k)"),
...
// MARK: - Counting Sort Step Information Panel
struct CountingSortStepInformationPanel: View {
    let currentPass: Int
    let currentStep: Int
                    ...
                    if !comparisonResult.isEmpty {
                        ...
                        HStack {
                            Image(systemName: comparisonResult.contains("⚡") ? "bolt.circle.fill" : 
                                           comparisonResult.contains("📊") ? "chart.bar.fill" :
                                           comparisonResult.contains("🔢") ? "number.circle.fill" :
                                           comparisonResult.contains("🔄") ? "arrow.triangle.2.circlepath.circle.fill" :
                                           comparisonResult.contains("🏗️") ? "hammer.circle.fill" : "checkmark.circle.fill")
                                .foregroundColor(comparisonResult.contains("⚡") ? .orange : 
                                               comparisonResult.contains("📊") ? .cyan :
                                               comparisonResult.contains("🔢") ? .purple :
                                               comparisonResult.contains("🔄") ? .blue :
                                               comparisonResult.contains("🏗️") ? .indigo : .green)
...
// MARK: - Counting Sort Array Element View
struct CountingSortArrayElementView: View {
    let value: Int
    let index: Int
    let state: CountingSortViewModel.ElementState
...
// MARK: - Count Array Element View
struct CountArrayElementView: View {
    let index: Int
    let count: Int
    ...
    var body: some View {
        VStack(spacing: 4) {
            Text("\(count)")
                ...
                .foregroundColor(isHighlighted ? .purple : Theme.Colors.primaryText)
            
            RoundedRectangle(cornerRadius: 6)
                .fill(isHighlighted ? Color.purple.opacity(0.15) : Color.cyan.opacity(0.1))
                .frame(width: 35, height: 40)
                .overlay(
                    RoundedRectangle(cornerRadius: 6)
                        .stroke(isHighlighted ? Color.purple : Color.cyan, lineWidth: isHighlighted ? 2.5 : 1.5)
                )
            
            Text("[\(index)]")
                .font(.system(size: 10, weight: .semibold, design: .monospaced))
                .foregroundColor(isHighlighted ? .purple : Theme.Colors.secondaryText.opacity(0.7))
        }
